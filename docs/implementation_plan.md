...
- **검증 목록**:
    - [ ] 로그인 페이지가 모바일/데스크탑에서 프리미엄 감성을 유지하는가?
    - [ ] 로딩 상태(Loading/Pending)에 따른 시각적 피드백이 명확한가?
...
- **검증 목록**:
    - [ ] 소셜 인증 성공 후 의도한 페이지(`/`)로 정확히 리다이렉트 되는가?
    - [ ] 인증 실패 시 사용자에게 적절한 에러 메시지를 표시하는가?
...
- **검증 목록**:
    - [ ] `/chat`, `/concierge` 등 보호된 페이지를 비로그인 상태에서 접근 차단하는가?
    - [ ] 로그인 후 이전 시도했던 페이지로 정확히 복귀하는가?
...
- **검증 목록**:
    - [ ] 소셜 프로필 이미지 변경 시 DB에 정상 동기화되는가?
    - [ ] 신규 가입자와 기존 사용자의 구분이 정확하게 처리되는가?
