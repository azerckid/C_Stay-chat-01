...
- **검증 목록**:
    - [ ] 메시지 시간이 '오후 2:30' 형태로 올바르게 표시되는가?
    - [ ] 연속해서 보낸 메시지의 말풍선 간격이 좁혀지는가?
    - [ ] URL이 포함된 텍스트가 클릭 가능한 링크로 변환되는가?
...
- **검증 목록**:
    - [ ] 어제와 오늘 보낸 메시지 사이에 날짜 구분선이 렌더링되는가?
...
- **검증 목록**:
    ...
    - [ ] 스크롤을 올려서 과거 내용을 볼 때 새 메시지가 와도 튀지 않는가?
...
- **작업**:
    ...
    - 이미지 전송 시 프리뷰 UI 구현
- **검증 목록**:
    - [ ] 플러스 버튼 클릭 시 메뉴가 열리는가? (현재 버튼만 있음)
